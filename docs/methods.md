...
- 2026-04-18 — Chunk C landed: `runIsoscopeGene()` + `classifyIsoformsBySiteUsage()`. Subprocess-calls isoscope with `--config <path>` (upstream change in isoscope at `6ade156`) so each source has its own config; `inst/isoscope_configs/haec185_basal.R` + `nmd_lungcells.R`. Per-source `ISOVAR_SOURCE_META` propagates into the output's `isovar_meta`. User-level secrets at `~/.config/isovar/secrets.env` via `loadIsovarSecrets()`.
- 2026-04-18 — Chunk C addendum: optional pre-SQANTI isocall mode added as an upstream change to isoscope (`SOURCE_KIND = "isocall"`, commit `157f121`), consumed via `inst/isoscope_configs/haec185_basal_isocall.R` + `nmd_lungcells_isocall.R`. Enumerates isoforms directly from a tabix-indexed isocall GTF without a SQANTI classification file. Tags `source = "ISOCALL"` in the output. Confirmed that AKR1A1 shifted splice sites are absent from both SQANTI-corrected AND raw isocall data in both cohorts — the predicted-but-unobserved finding is not a SQANTI correction artifact.
- 2026-04-18 — Chunk D landed: `longreadSources()`, `parseSampleColumns()`, `loadLongreadEvidence()` for multi-source long-read evidence with per-condition stratification. Includes the short-read Leafcutter sQTL input (`loadSqtlResults()` / `annotateSqtl()`) — per-variant × per-intron β/SE/p from the 185-donor cohort, contrary to the earlier assumption the indels ARE included. Biological finding from the AKR1A1 NMD contrast: phenotype-intron reads go from 951 (DMSO, n=3) to 9887 (Smg1i, n=3) in DD_ALI (10.4× rescue), with DD (non-ALI) essentially unchanged — direct evidence these isoforms are NMD-targeted in a differentiation-state-dependent manner.
- 2026-04-18 — isoscope upstream dedup fix at `1f447de` — isoforms that appear in both GENCODE reference and the long-read caller's output are emitted once (GENCODE row retained, `detected_in_longread` column flags long-read support).
- 2026-04-18 — Chunk E landed: Isopair interface. `parseStructuresMultiGtf()`, `extractCdsMultiGtf()`, `loadTranscriptSequences()`, `hiddenPtcStatus()` (wraps `Isopair::traceReferenceAtg()`), `computeDominantIsoform()` (wraps `Isopair::identifyDominantIsoforms()`). AKR1A1 validation: standard PTC analysis finds **0 / 32** coding isoforms as PTC+; the hidden-PTC trace from the reference ATG identifies **16 effectively_ptc** + **16 ref_atg_lost** + **22 truncated_no_ejc** comparators. Cross-check with the DD_ALI Smg1i rescue signal: the top 10 isoforms by Smg1i/DMSO ratio are ALL either `effectively_ptc` or `ref_atg_lost` or `no_downstream_ejc` (15–51× rescue). The hidden-PTC attribution directly explains the Smg1i-rescued isoforms that standard CDS-based PTC detection missed entirely.
- 2026-04-18 — Chunk E.1: Isopair upstream extension (commit `63ff9ae`). `traceReferenceAtg` gains `resolve_alt_start` + `min_alt_orf_nt` parameters. When the reference ATG isn't exonic in the comparator, the function now scans the comparator for the first viable ATG (ORF ≥ 30 nt by default) and traces from there — producing concrete `alt_start_effectively_ptc` or `alt_start_no_downstream_ejc` verdicts instead of the terminal `ref_atg_lost`. isovar's `hiddenPtcStatus()` defaults to `resolve_alt_start = TRUE`. AKR1A1 result: of the 16 former `ref_atg_lost` comparators, **14 resolve to `alt_start_effectively_ptc`** (NMD substrate via alt start), **2 to `alt_start_no_downstream_ejc`** (translated to a last-exon stop, not NMD-triggering). Every top-10 Smg1i-rescued isoform now has an explicit hidden-PTC verdict.
- 2026-04-18 — Chunk E.2: per-ORF classifications via `Isopair::enumerateOrfs` (upstream `242b443`) + isovar wrapper `enumerateComparatorOrfs` + `summarizeOrfsToTranscript`. Returns one row per (isoform, viable ORF) with category in `{effectively_ptc, no_downstream_ejc, no_stop_in_frame}`. Per-ORF analysis replaces per-transcript analysis as the biologically correct unit — a transcript can host multiple ORFs with independent fates. Level 3 (per-ORF canonical across all Isopair functions) deferred and tracked at Isopair#1 / Isovar#2.
- 2026-04-18 — Chunk E.2 Kozak gating (upstream `92ebddb`). `enumerateOrfs` defaults to scoring every ATG's Kozak context with Isopair's PWM and retaining only those at/above `kozak_threshold = 0` (log-odds above random) before ORF tracing. Most ATGs in a transcript have weak Kozak context and wouldn't initiate translation; gating reduces AKR1A1's top-10-rescued-isoform ORF count from 284 → 65 (~5× noise reduction) while preserving every NMD signal. `Isopair::empiricalKozakThreshold(structures, cds, sequences, quantile)` derives a data-driven threshold from the distribution of Kozak scores at annotated CDS starts — pass `$threshold` as `kozak_threshold` to use "at least as good as (1-quantile)% of annotated CDS starts" as the gate.
