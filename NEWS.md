...
- `hiddenPtcStatus()` default changed to `resolve_alt_start = TRUE` (Chunk E.1). Requires Isopair ≥ 0.99.5 (upstream commit `63ff9ae`) — the `ref_atg_lost` terminal category is replaced by concrete `alt_start_effectively_ptc` / `alt_start_no_downstream_ejc` / `ref_atg_lost_no_viable_start` verdicts derived from tracing the first viable alt-start ORF in the comparator. For AKR1A1: 14 of 16 former `ref_atg_lost` comparators resolve to `alt_start_effectively_ptc` — NMD substrate via alt start.
- `enumerateComparatorOrfs()` / `summarizeOrfsToTranscript()` — per-ORF classifications. Wrap `Isopair::enumerateOrfs` (Chunk E.2). Kozak-gated by default (`kozak_filter = TRUE`, `kozak_threshold = 0`) — only ATGs with initiation-competent context are traced. Pass a data-driven threshold from `Isopair::empiricalKozakThreshold()` for analyses where the threshold should match the training-set distribution.
